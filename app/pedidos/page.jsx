...
function getDeliveryStatus(order) {
  const d = Array.isArray(order.atacarejo_deliveries) ? order.atacarejo_deliveries[0] : order.atacarejo_deliveries;
...
function PedidosContent() {
  ...
  async function loadOrders() {
    ...
    let query = supabase
      ...
      .select("*, atacarejo_customers(name, email, phone, street, street_number, complement, neighborhood, city, state, zip_code, reference_point), atacarejo_order_items(product_name, unit_price, quantity), atacarejo_deliveries(status, tracking_code, carrier, estimated_date, data_entrega)")
  ...
  const filteredOrders = atacarejo_orders.filter(
          ...
          <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}>
              <div style={styles.modalSection}>
                ...
                <p style={styles.modalText}>{detailOrder.atacarejo_customers?.name || "—"}</p>
                <p style={styles.modalTextMuted}>{detailOrder.atacarejo_customers?.phone} {detailOrder.atacarejo_customers?.email ? `· ${detailOrder.atacarejo_customers.email}` : ""}</p>
              </div>

              <div style={styles.modalSection}>
                <span style={styles.modalSectionTitle}>Endereço de entrega</span>
                {enderecoFormatado(detailOrder.atacarejo_customers).map((linha, i) => (
                  <p key={i} style={styles.modalText}>{linha}</p>
                ))}
                {detailOrder.atacarejo_customers?.reference_point && (
                  <p style={styles.modalTextMuted}>Referência: {detailOrder.atacarejo_customers.reference_point}</p>
                )}
              </div>

              <div style={styles.modalSection}>
                <span style={styles.modalSectionTitle}>Itens</span>
                {(detailOrder.atacarejo_order_items || []).map((item, i) => (
              ...
              <div style={styles.modalSection}>
                ...
                <p style={styles.modalText}>
                  ...
                  {detailOrder.atacarejo_delivery_payment_options?.length > 0 && (
                    ` — ${detailOrder.atacarejo_delivery_payment_options.map((id) => DELIVERY_PAYMENT_LABELS[id] || id).join(" + ")}`
                  )}
                </p>
              ...
              <div style={styles.modalSection}>
                ...
                {(() => {
                  const d = Array.isArray(detailOrder.atacarejo_deliveries) ? detailOrder.atacarejo_deliveries[0] : detailOrder.atacarejo_deliveries;
                  if (!d) return null;
                  return (
                    <>
