...
function produtoFavorito(atacarejo_orders) {
  const contagem = {};
...
function SacContent() {
  ...
  async function loadTickets() {
    ...
    const [ticketsResult, reclamacoesResult, customersResult] = await Promise.all([
      supabase.from("atacarejo_support_tickets").select("*, atacarejo_orders(order_number)").order("created_at", { ascending: false }),
      supabase.from("atacarejo_juridico_reclamacoes").select("id, sac_chamado_id, canal, status").not("sac_chamado_id", "is", null),
      supabase.from("atacarejo_customers").select("id, phone, street, street_number, neighborhood, city, state, atacarejo_orders(created_at, total, atacarejo_order_items(product_name, quantity))"),
    ]);
    ...
    const atacarejo_customers = customersResult.data || [];
    const byPhone = {};
    atacarejo_customers.forEach((c) => {
      const tel = normalizarTelefone(c.phone);
      if (!tel) return;
      const atacarejo_orders = c.atacarejo_orders || [];
      const totalGasto = atacarejo_orders.reduce((s, o) => s + Number(o.total), 0);
      const ultimaCompra = atacarejo_orders.length > 0
        ? atacarejo_orders.reduce((max, o) => (new Date(o.created_at) > new Date(max) ? o.created_at : max), atacarejo_orders[0].created_at)
        : null;
      ...
      const favorito = produtoFavorito(atacarejo_orders);
      const perfil = classificarCliente(atacarejo_orders.length, diasDesdeUltimaCompra ?? 9999, totalGasto, periodicidade);

      byPhone[tel] = {
        total_pedidos: atacarejo_orders.length,
        total_gasto: totalGasto,
        ticket_medio: atacarejo_orders.length > 0 ? totalGasto / atacarejo_orders.length : 0,
  ...
  async function handleSelecionarCliente(cliente) {
    ...
    const { data } = await supabase
      ...
      .select("order_number, created_at, total, status, atacarejo_order_items(product_name, quantity)")
  ...
  async function handleCreate(e) {
    ...
    await supabase.from("atacarejo_support_tickets").insert({
  ...
  async function handleStatusChange(id, newStatus) {
    ...
    await supabase.from("atacarejo_support_tickets").update(payload).eq("id", id);
    loadTickets();
  }
  ...
  async function handleResponseSave(id, value) {
    await supabase.from("atacarejo_support_tickets").update({ response: value, updated_at: new Date().toISOString() }).eq("id", id);
  }
