...
function RhContent() {
  ...
  async function loadEmployees() {
    setLoading(true);
    const { data } = await supabase.from("atacarejo_employees").select("*").order("name");
    setEmployees(data || []);
    setLoading(false);
  }
  ...
  async function handleCreate(e) {
    ...
    await supabase.from("atacarejo_employees").insert({
      name: name.trim(),
      role: role.trim() || null,
      department,
  ...
  async function handleToggleStatus(id, status) {
    await supabase.from("atacarejo_employees").update({ status: status === "ativo" ? "inativo" : "ativo" }).eq("id", id);
    loadEmployees();
  }

  async function handleDelete(id, empName) {
    if (!confirm(`Remover "${empName}" da equipe?`)) return;
    await supabase.from("atacarejo_employees").delete().eq("id", id);
    loadEmployees();
  }
  ...
  const counts = DEPARTMENTS.reduce((acc, d) => {
    acc[d] = atacarejo_employees.filter((e) => e.department === d && e.status === "ativo").length;
    return acc;
  }, {});

  const filtered = atacarejo_employees.filter((e) => departmentFilter === "todos" || e.department === departmentFilter);
      ...
      <div style={styles.content}>
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.subtitle}>{atacarejo_employees.filter((e) => e.status === "ativo").length} pessoas ativas na equipe</p>
          </div>
