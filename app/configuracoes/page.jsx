...
function ConfiguracoesContent() {
  ...
  async function handleCreateRule(e) {
    ...
    const { error } = await supabase.from("atacarejo_shipping_rules").insert({
      cep_start: cepStart,
      cep_end: cepEnd,
      mode: ruleMode,
  ...
  async function handleToggleRule(id, currentEnabled) {
    ...
    await supabase.from("atacarejo_shipping_rules").update({ active: !currentEnabled, updated_at: new Date().toISOString() }).eq("id", id);
  }

  async function handleDeleteRule(id) {
    if (!confirm("Excluir essa faixa de frete? Essa ação não pode ser desfeita.")) return;
    await supabase.from("atacarejo_shipping_rules").delete().eq("id", id);
    loadSettings();
  }
  ...
  async function loadSettings() {
    ...
    const [payments, modules, delivery, shipping] = await Promise.all([
      supabase.from("atacarejo_payment_settings").select("*").order("id"),
      supabase.from("atacarejo_module_settings").select("*").order("id"),
      supabase.from("atacarejo_delivery_payment_options").select("*").order("sort_order"),
      supabase.from("atacarejo_shipping_rules").select("*").order("cep_start"),
    ]);
  ...
  async function handleTogglePayment(id, currentEnabled) {
    ...
    await supabase
      .from("atacarejo_payment_settings")
  ...
  async function handleToggleModule(id, currentEnabled) {
    ...
    await supabase
      .from("atacarejo_module_settings")
  ...
  async function handleToggleDeliveryOption(id, currentEnabled) {
    ...
    await supabase.from("atacarejo_delivery_payment_options").update({ enabled: !currentEnabled }).eq("id", id);
  }
  ...
  async function handleCreateDeliveryOption(e) {
    ...
    const { error } = await supabase.from("atacarejo_delivery_payment_options").insert({
