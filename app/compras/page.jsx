...
function ComprasContent() {
  ...
  const [atacarejo_suppliers, setSuppliers] = useState([]);
  ...
  async function loadAll() {
    ...
    const [s, i, p] = await Promise.all([
      supabase.from("atacarejo_suppliers").select("*").order("name"),
      supabase.from("atacarejo_purchase_invoices").select("*, atacarejo_suppliers(name)").order("created_at", { ascending: false }),
      supabase.from("atacarejo_products").select("id, name, sku, stock, cost_price").order("name"),
    ]);
    setSuppliers(s.data || []);
    setInvoices(i.data || []);
    setProducts(p.data || []);
  ...
  async function handleCreateSupplier(e) {
    ...
    await supabase.from("atacarejo_suppliers").insert({
      name: supName.trim(),
  ...
  async function handleToggleSupplierActive(id, active) {
    await supabase.from("atacarejo_suppliers").update({ active: !active, updated_at: new Date().toISOString() }).eq("id", id);
    loadAll();
  }
  ...
  async function handleCreateInvoice(e) {
    ...
    const { data: invoice, error: invoiceError } = await supabase
      .from("atacarejo_purchase_invoices")
      .insert({
    ...
    if (!invoiceError && invoice) {
      ...
      await supabase.from("atacarejo_purchase_invoice_items").insert(itemsPayload);
    }
  ...
  async function handleLaunchInvoice(id) {
    ...
    setLaunchingId(id);
    const { error } = await supabase.rpc("atacarejo_launch_purchase_invoice", { invoice_id: id });
                ...
                <div style={styles.row}>
                  <div style={{ flex: 1 }}>
                    ...
                    <select value={invSupplierId} onChange={(e) => setInvSupplierId(e.target.value)} style={styles.input}>
                      ...
                      {atacarejo_suppliers.filter((s) => s.active).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                  </div>
                  <div style={{ flex: 1 }}>
                ...
                {items.map((it, index) => (
                  <div key={index} style={styles.itemRow}>
                    ...
                    >
                      ...
                      {atacarejo_products.map((p) => <option key={p.id} value={p.id}>{p.name} {p.sku ? `(${p.sku})` : ""}</option>)}
                    </select>
                    <input
                  ...
                  <tbody>
                    {invoices.map((inv) => (
                      <tr key={inv.id} style={styles.tr}>
                        ...
                        <td style={styles.td}>{inv.atacarejo_suppliers?.name || "—"}</td>
