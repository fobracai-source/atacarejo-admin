...
function FinanceiroContent() {
  ...
  async function handleTransfer(e) {
    ...
    const rpcName = showTransferForm === "para_caixa" ? "atacarejo_transferir_banco_para_caixa" : "atacarejo_transferir_caixa_para_banco";
  ...
  async function loadGovernoData() {
    ...
    const { data: customersData } = customerIds.length
      ? await supabase.from("atacarejo_customers").select("id, name, phone").in("id", customerIds)
      : { data: [] };
  ...
  async function loadOrderDetailGoverno(orderId) {
    const { data } = await supabase
      ...
      .select("*, atacarejo_customers(name, phone, email, street, street_number, complement, neighborhood, city, state, zip_code, reference_point), atacarejo_order_items(product_name, unit_price, quantity, product_id, atacarejo_products(tax_pct)), atacarejo_deliveries(status)")
      .eq("id", orderId)
      .single();
  ...
  async function handleGerarContaGoverno() {
    ...
    const { error } = await supabase.rpc("atacarejo_gerar_conta_imposto_governo", { p_order_ids: selectedOrderIds });
  ...
  async function handleGerarFolha() {
    ...
    setMensagemFolha("");
    const { error } = await supabase.rpc("atacarejo_gerar_folha_pagamento", { p_forcar: false });

    if (error) {
      if (error.message.includes("FOLHA_JA_GERADA")) {
        const mesAtual = new Date().toISOString().slice(0, 7);
        const { data: runData } = await supabase.from("atacarejo_payroll_runs").select("id").eq("mes", mesAtual).maybeSingle();
  ...
  async function handleDuplicarFolha() {
    ...
    const { error } = await supabase.rpc("atacarejo_gerar_folha_pagamento", { p_forcar: true });
  ...
  async function handleApagarEGerarFolha() {
    ...
    setProcessandoConflito(true);
    const { error: deleteError } = await supabase.rpc("atacarejo_apagar_folha_mes", { p_run_id: folhaConflictRunId });
    if (deleteError) {
  ...
  async function loadTransactions() {
    setLoading(true);
    const { data } = await supabase.from("atacarejo_financial_transactions").select("*").order("due_date", { ascending: true });
    setTransactions(data || []);
    setLoading(false);
  }

  async function loadChartOfAccounts() {
    const { data } = await supabase.from("atacarejo_chart_of_accounts").select("*").order("name");
  ...
  async function handleCreateAccount(e) {
    ...
    await supabase.from("atacarejo_chart_of_accounts").insert({
  ...
  async function handleUpdateAccount(id, field, value) {
    ...
    await supabase.from("atacarejo_chart_of_accounts").update({ [field]: value }).eq("id", id);
  }
  ...
  async function handleCreate(e) {
    ...
    setSaving(true);
    await supabase.from("atacarejo_financial_transactions").insert({
  ...
  async function handleBaixaIndividual(id) {
    await supabase.from("atacarejo_financial_transactions").update({ status: "baixado", payment_date: todayDateInput() }).eq("id", id);
    loadTransactions();
  }
  ...
  async function handleBaixaEmLote() {
    ...
    await supabase.from("atacarejo_financial_transactions")
  ...
  async function calculateDRE() {
    ...
    let query = supabase.from("atacarejo_financial_transactions").select("*");
    if (dreRegime === "caixa") {
    ...
    if (!modoComissaoAtivo && orderIds.length > 0) {
      ...
      const { data: atacarejo_products } = await supabase
        .from("atacarejo_products")
        .select("id, cost_price")
        .in("id", productIds);

      const costMap = {};
      (atacarejo_products || []).forEach((p) => { costMap[p.id] = Number(p.cost_price) || 0; });
    ...
    <div>
      ...
      <div style={styles.content}>
        ...
        {tab === "bancos" && (
          <>
            {bancoLoading ? (
              ...
              <p style={styles.empty}>Nenhuma conta encontrada no Banco Alegre para a Atacarejo ainda.</p>
            ) : (
              <>
                <div style={styles.statsRow}>
        ...
        {tab === "investimentos" && (
          <>
            {!contaBanco ? (
              <p style={styles.empty}>Abra a aba "Bancos" primeiro para carregar a conta da Atacarejo.</p>
            ) : (
              <>
                ...
                <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
                  ...
                  <div style={styles.modalBody}>
                    <div style={styles.modalSection}>
                      ...
                      <p style={styles.modalText}>{detailOrderGoverno.atacarejo_customers?.name || "—"}</p>
                      <p style={styles.modalTextMuted}><Phone size={11} style={{ display: "inline", marginRight: 4 }} />{detailOrderGoverno.atacarejo_customers?.phone} {detailOrderGoverno.atacarejo_customers?.email ? `· ${detailOrderGoverno.atacarejo_customers.email}` : ""}</p>
                    </div>
                    <div style={styles.modalSection}>
                      <span style={styles.modalSectionTitle}><MapPin size={11} style={{ display: "inline", marginRight: 4 }} />Endereço</span>
                      <p style={styles.modalText}>
                        {[detailOrderGoverno.atacarejo_customers?.street, detailOrderGoverno.atacarejo_customers?.street_number].filter(Boolean).join(", ")}
                        {detailOrderGoverno.atacarejo_customers?.complement ? ` — ${detailOrderGoverno.atacarejo_customers.complement}` : ""}
                      </p>
                      <p style={styles.modalText}>{detailOrderGoverno.atacarejo_customers?.neighborhood}</p>
                      <p style={styles.modalText}>{[detailOrderGoverno.atacarejo_customers?.city, detailOrderGoverno.atacarejo_customers?.state].filter(Boolean).join(" - ")} {detailOrderGoverno.atacarejo_customers?.zip_code ? `· CEP ${detailOrderGoverno.atacarejo_customers.zip_code}` : ""}</p>
                    </div>
                    <div style={styles.modalSection}>
                      <span style={styles.modalSectionTitle}>Itens e imposto por item</span>
                      {(detailOrderGoverno.atacarejo_order_items || []).map((item, i) => {
                        const taxPct = item.atacarejo_products?.tax_pct || 0;
                    ...
                    <div style={styles.modalSection}>
                      ...
                      <p style={styles.modalText}>{Array.isArray(detailOrderGoverno.atacarejo_deliveries) ? detailOrderGoverno.atacarejo_deliveries[0]?.status : detailOrderGoverno.atacarejo_deliveries?.status}</p>
                    </div>
