...
function formaPagamentoMaisUsada(atacarejo_orders) {
  if (!atacarejo_orders || atacarejo_orders.length === 0) return null;
  const contagem = {};
  atacarejo_orders.forEach((o) => { contagem[o.payment_method] = (contagem[o.payment_method] || 0) + 1; });
...
function produtosFavoritos(atacarejo_orders) {
  const contagem = {};
  atacarejo_orders.forEach((o) => {
    (o.atacarejo_order_items || []).forEach((item) => {
...
function calcularPeriodicidade(atacarejo_orders) {
  if (atacarejo_orders.length < 2) return null;
  const datas = atacarejo_orders.map((o) => new Date(o.created_at)).sort((a, b) => a - b);
  let somaDias = 0;
...
function CustomerDetailContent() {
  ...
  const [atacarejo_interactions, setInteractions] = useState([]);
  ...
  async function loadAll() {
    ...
    const [{ data: customerData }, { data: orderData }, { data: interactionData }] = await Promise.all([
      supabase.from("atacarejo_customers").select("*").eq("id", id).single(),
      supabase.from("atacarejo_orders").select("*, atacarejo_order_items(product_name, unit_price, quantity)").eq("customer_id", id).order("created_at", { ascending: false }),
      supabase.from("atacarejo_interactions").select("*").eq("customer_id", id).order("created_at", { ascending: false }),
    ]);
    ...
    if (customerData?.phone) {
      ...
      const { data: allTickets } = await supabase.from("atacarejo_support_tickets").select("id, subject, status, priority, created_at, customer_phone");
  ...
  async function handleStageChange(stage) {
    await supabase.from("atacarejo_customers").update({ stage }).eq("id", id);
  ...
  async function handleAddInteraction(e) {
    ...
    await supabase.from("atacarejo_interactions").insert({
      customer_id: id,
      type: newType,
  ...
  async function handleDeleteInteraction(interactionId) {
    await supabase.from("atacarejo_interactions").delete().eq("id", interactionId);
    loadAll();
  }
  ...
  const totalGasto = atacarejo_orders.reduce((sum, o) => sum + Number(o.total), 0);
  const ticketMedio = atacarejo_orders.length > 0 ? totalGasto / atacarejo_orders.length : 0;
  const ultimaCompra = atacarejo_orders.length > 0 ? atacarejo_orders[0].created_at : null;
  const diasDesdeUltimaCompra = ultimaCompra ? Math.round((Date.now() - new Date(ultimaCompra)) / (1000 * 60 * 60 * 24)) : null;
  const pagamentoPreferido = formaPagamentoMaisUsada(atacarejo_orders);
  const periodicidade = calcularPeriodicidade(atacarejo_orders);
  const favoritos = produtosFavoritos(atacarejo_orders);
  const perfil = classificarCliente(atacarejo_orders.length, diasDesdeUltimaCompra ?? 9999, totalGasto, periodicidade);
      ...
      <div style={styles.content}>
        ...
        <div style={styles.statsRow}>
          <div style={styles.statCard}>
            <div style={styles.statValue}>{atacarejo_orders.length}</div>
          ...
          <div style={styles.infoCard}>
            ...
            {pagamentoPreferido ? (
              <>
                ...
                <p style={styles.infoCardTextMuted}>Usada em {pagamentoPreferido.vezes} de {atacarejo_orders.length} pedido(s)</p>
              </>
            ) : (
                ...
                <div key={o.id}>
                  ...
                  {expandedOrder === o.id && (
                    <div style={styles.orderItemsBox}>
                      {(o.atacarejo_order_items || []).map((item, idx) => (
