...
function CrmContent() {
  const [atacarejo_customers, setCustomers] = useState([]);
  ...
  async function loadCustomers() {
    ...
    const [{ data: customersData }, { data: ordersData }] = await Promise.all([
      supabase.from("atacarejo_customers").select("*").order("created_at", { ascending: false }),
      supabase.from("atacarejo_orders").select("customer_id, total"),
    ]);
  ...
  async function handleCreate(e) {
    ...
    await supabase.from("atacarejo_customers").insert({
      name: newName,
      email: newEmail || null,
      phone: newPhone || null,
  ...
  const filtered = atacarejo_customers.filter((c) =>
      ...
      <div style={styles.content}>
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.subtitle}>{atacarejo_customers.length} pessoas cadastradas</p>
          </div>
