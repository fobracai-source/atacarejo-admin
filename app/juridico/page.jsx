...
function JuridicoContent() {
  ...
  const [atacarejo_employees, setEmployees] = useState([]);
  ...
  async function loadPolitica() {
    ...
    const { data } = await supabase.from("atacarejo_juridico_lgpd_politica").select("*").eq("id", "atual").maybeSingle();
  ...
  async function handleSalvarPolitica() {
    ...
    await supabase.from("atacarejo_juridico_lgpd_politica").upsert({
      id: "atual",
      texto: politicaTexto,
  ...
  async function loadAnexos() {
    ...
    const { data } = await supabase.from("atacarejo_juridico_anexos").select("*").order("created_at", { ascending: false });
    setAnexos(data || []);
  ...
  async function handleUploadAnexo(e) {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("atacarejo-juridico-anexos").upload(filePath, anexoArquivo);
      if (uploadError) throw uploadError;

      const { data: publicUrlData } = supabase.storage.from("atacarejo-juridico-anexos").getPublicUrl(filePath);

      const { error: insertError } = await supabase.from("atacarejo_juridico_anexos").insert({
  ...
  async function handleDeleteAnexo(id, titulo) {
    ...
    await supabase.from("atacarejo_juridico_anexos").delete().eq("id", id);
    loadAnexos();
  }
  // ────────────────── FIM ABA ANEXOS ──────────────────
  ...
  async function loadAll() {
    ...
    const [c, o, l, r, d, e, s] = await Promise.all([
      supabase.from("atacarejo_juridico_contratos").select("*").order("data_fim", { ascending: true, nullsFirst: false }),
      supabase.from("atacarejo_juridico_obrigacoes_fiscais").select("*").order("vencimento", { ascending: true }),
      supabase.from("atacarejo_juridico_lgpd_solicitacoes").select("*").order("prazo_resposta", { ascending: true }),
      supabase.from("atacarejo_juridico_reclamacoes").select("*").order("data_reclamacao", { ascending: false }),
      supabase.from("atacarejo_juridico_documentos").select("*").order("data_validade", { ascending: true, nullsFirst: false }),
      supabase.from("atacarejo_employees").select("id, name, department"),
      supabase.from("atacarejo_support_tickets").select("id, subject, customer_name, status").order("created_at", { ascending: false }),
    ]);
    setContratos(c.data || []);
  ...
  const employeeMap = useMemo(() => {
    const map = {};
    atacarejo_employees.forEach((e) => { map[e.id] = e.name; });
    return map;
  }, [atacarejo_employees]);
  ...
  async function handleCreateContrato(e) {
    ...
    await supabase.from("atacarejo_juridico_contratos").insert({
  ...
  async function handleCreateFiscal(e) {
    ...
    await supabase.from("atacarejo_juridico_obrigacoes_fiscais").insert({
      nome: fsNome.trim(),
      tipo: fsTipo,
  ...
  async function handleCreateLgpd(e) {
    ...
    await supabase.from("atacarejo_juridico_lgpd_solicitacoes").insert({
  ...
  async function handleCreateReclamacao(e) {
    ...
    await supabase.from("atacarejo_juridico_reclamacoes").insert({
  ...
  async function handleCreateDocumento(e) {
    ...
    await supabase.from("atacarejo_juridico_documentos").insert({
      nome: dcNome.trim(),
      tipo: dcTipo,
                    ...
                    {ctTipo === "funcionario" ? (
                      <>
                        ...
                        <select value={ctFuncionarioId} onChange={(e) => setCtFuncionarioId(e.target.value)} style={styles.input} required>
                          ...
                          {atacarejo_employees.map((emp) => <option key={emp.id} value={emp.id}>{emp.name} — {emp.department}</option>)}
                        </select>
                        {atacarejo_employees.length === 0 && <p style={styles.warnNote}>Nenhum funcionário cadastrado no RH ainda.</p>}
                      </>
                    ) : (
                      <>
                        ...
                        {contratos.map((c) => {
                          ...
                          return (
                            <tr key={c.id} style={{ ...styles.tr, ...(vencendo ? styles.trOverdue : {}) }}>
                              ...
                              <td style={styles.td}>
                                <select value={c.status} onChange={(e) => handleUpdateStatus("atacarejo_juridico_contratos", c.id, e.target.value)} style={statusBadgeStyle(c.status)}>
                      ...
                      <tbody>
                        {obrigacoes.map((o) => (
                          <tr key={o.id} style={styles.tr}>
                            ...
                            <td style={styles.td}>
                              <select value={o.status} onChange={(e) => handleUpdateStatus("atacarejo_juridico_obrigacoes_fiscais", o.id, e.target.value)} style={statusBadgeStyle(o.status)}>
            ...
            {tab === "lgpd" && (
              <>
                ...
                <p style={styles.explainer}>
                  ...
                  Esse texto aparece publicamente nos dois sites (Atacarejo e Atacarejo), numa página
                        ...
                        {lgpd.map((l) => {
                          ...
                          return (
                            <tr key={l.id} style={{ ...styles.tr, ...(estourado ? styles.trOverdue : {}) }}>
                              ...
                              <td style={styles.td}>
                                <select value={l.status} onChange={(e) => handleUpdateStatus("atacarejo_juridico_lgpd_solicitacoes", l.id, e.target.value)} style={statusBadgeStyle(l.status)}>
                        ...
                        {reclamacoes.map((r) => {
                          ...
                          return (
                            <tr key={r.id} style={styles.tr}>
                              ...
                              <td style={styles.td}>
                                <select value={r.status} onChange={(e) => handleUpdateStatus("atacarejo_juridico_reclamacoes", r.id, e.target.value)} style={statusBadgeStyle(r.status)}>
