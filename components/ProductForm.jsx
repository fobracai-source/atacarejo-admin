...
export default function ProductForm({ initialProduct = null }) {
  ...
  useEffect(() => {
    async function loadSuggestions() {
      const { data } = await supabase.from("atacarejo_products").select("category, brand");
    ...
    try {
      ...
      for (const photo of photos.slice(0, MAX_PHOTOS)) {
        if (photo.file) {
          ...
          const { error: uploadError } = await supabase.storage.from("atacarejo-product-images").upload(filePath, photo.file);
          if (uploadError) throw uploadError;
          const { data: publicUrlData } = supabase.storage.from("atacarejo-product-images").getPublicUrl(filePath);
      ...
      if (isEditing) {
        const { error: updateError } = await supabase.from("atacarejo_products").update(payload).eq("id", initialProduct.id);
        if (updateError) throw updateError;
      } else {
        const { error: insertError } = await supabase.from("atacarejo_products").insert(payload);
