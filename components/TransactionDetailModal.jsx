...
export default function TransactionDetailModal({ transaction, chartOfAccounts = [], onClose, onChanged }) {
  ...
  useEffect(() => {
    async function loadOrder() {
      ...
      const { data } = await supabase
        ...
        .select("*, atacarejo_customers(name, phone, address), atacarejo_order_items(product_name, quantity, unit_price)")
  ...
  async function handleSave() {
    ...
    const { error: updateError } = await supabase.from("atacarejo_financial_transactions").update(payload).eq("id", transaction.id);
  ...
  async function handleDelete() {
    ...
    await supabase.from("atacarejo_financial_transactions").delete().eq("id", transaction.id);
    onChanged?.();
    onClose();
  }
                ...
                <div style={styles.orderSection}>
                  ...
                  {loadingOrder ? (
                    ...
                    <>
                      ...
                      <div style={styles.row}><span style={styles.label}>Cliente</span><span style={styles.value}>{order.atacarejo_customers?.name || "—"}</span></div>
                      {order.atacarejo_customers?.phone && <div style={styles.row}><span style={styles.label}>Telefone</span><span style={styles.value}>{order.atacarejo_customers.phone}</span></div>}
                      {order.atacarejo_customers?.address && <div style={styles.row}><span style={styles.label}>Endereço</span><span style={styles.value}>{order.atacarejo_customers.address}</span></div>}
                      <div style={{ marginTop: 8 }}>
                        <span style={styles.label}>Itens</span>
                        <ul style={styles.itemsList}>
                          {order.atacarejo_order_items?.map((item, i) => (
